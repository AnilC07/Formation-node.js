...
var fs = require('fs'); // ON LIT LE FICHIER QUI CONTIENTS LES DATAS DE MANIERES SYNCHRONE ET ON LES AFFECTE A LA CONSTANTE EN LES PARSANT AU FORMAT JSON


var tours = JSON.parse(fs.readFileSync("".concat(__dirname, "/../dev-data/data/tours-simple.json"))); // Creation d'un middleware (generale pour les routes qui prennent un id ) qui va checker si l'id est valide sur l'url
...
exports.getTour = function (req, res) {
  ...
  var id = Number(req.params.id); // ATTRIBUT LE PARAMETRE ID A UNE CONSTANTE EST LE CONVERTIT EN NOMBRE

  var tour = tours.find(function (el) {
    return el.id === id;
  }); // FIND() SUR LE TABLEAU. LA FONCTION RETOURNE TRUE OU FALSE. SEUL L'ELEMENT AYANT L'ID EGALE SERA RETOURNÉ
...
exports.createTour = function (req, res) {
  var newId = tours[tours.length - 1].id + 1; // ON DEFINIT UN NOUVELLE ID POUR NOTRE NOUVELLE DESTINATION
  // ON CONCATENE L'OBJET CONTENANT LE NOUVEL ID À L'OBJET RECUPERÉ DANS REQ.BODY

  var newTour = Object.assign({
    id: newId
  }, req.body); // AJOUTE L'OBJET NEWTOUR AU TABLEAU "TOURS"

  tours.push(newTour); // REECRIS SUR LE FICHIER CONTENANT LES DATAS AVEC LES NOUVELLES DATAS
